...
const getToken = async () => {
    ...
    const oauthResponse = await fetchJson(anonymOauth.toString(), {
        headers: {
...
const getVideo = async (ownerId, videoId, accessKey) => {
    return await fetchJson(videoApiUrl, {
        method: "POST",
        headers: {
...
export default async function ({ ownerId, videoId, accessKey, quality, subtitleLang }) {
    ...
    if (!Array.isArray(videoGet?.response?.items) || videoGet.response.items.length !== 1) {
