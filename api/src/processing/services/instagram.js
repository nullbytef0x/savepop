...
const mediaScore = (media) => {
    ...
    if (media.carousel_media?.length) return 5;
    if (media.video_versions?.length) return 4;
    if (media.image_versions2?.candidates?.length) return 3;
    if (media.display_uri) return 2;
    return 0;
}

export const extractPublicPageMedia = (html, id) => {
    let bestMedia, bestScore = 0;

    const visit = (value) => {
        if (!value || typeof value !== 'object') return;

        if (value.code === id) {
            const candidates = [value, value.if_not_gated_logged_out];
            for (const candidate of candidates) {
                const score = mediaScore(candidate);
                if (score > bestScore) {
                    bestMedia = candidate;
                    bestScore = score;
                }
...
export default function instagram(obj) {
    ...
    async function requestHTML(id, cookie) {
        ...
        const rawEmbedData = data?.match(/"init",\[\],\[(.*?)\]\],/)?.[1];
        if (!rawEmbedData) return;

        let embedData;
        try {
            embedData = JSON.parse(rawEmbedData);
        } catch {
            return;
        }
    ...
    async function requestPublicPage(id) {
        let bestMedia, bestScore = 0;
        ...
        for (const type of ['p', 'reel']) {
            for (let attempt = 0; attempt < 2; attempt++) {
                try {
                    ...
                    const media = extractPublicPageMedia(await response.text(), id);
                    const score = mediaScore(media);
                    if (score > bestScore) {
                        bestMedia = media;
                        bestScore = score;
                    }

                    // A video or carousel is already a complete result. An
                    // image may only be the poster for a reel, so keep looking.
                    if (score >= 4) return bestMedia;
                } catch {}
            }
        }
    ...
    function extractOldPost(data, id, alwaysProxy) {
        ...
        if (shortcodeMedia?.display_url && !shortcodeMedia?.is_video) {
            return {
    ...
    function extractNewPost(data, id, alwaysProxy) {
        ...
        } else if (data.image_versions2?.candidates && data.media_type !== 2) {
            return {
                urls: data.image_versions2.candidates[0].url,
                isPhoto: true,
                filename: `instagram_${id}.jpg`,
            }
        } else if (data.display_uri && data.media_type !== 2) {
            return {
                urls: data.display_uri,
                isPhoto: true,
                filename: `instagram_${id}.jpg`,
            }
        }
    ...
    async function getPost(id, alwaysProxy) {
        ...
        try {
            ...
            // Current logged-out pages include the post payload in JSON scripts
            // for search crawlers. This is independent of GraphQL document IDs.
            if (!hasData(data)) data = await requestPublicPage(id);
