...
политика конфиденциальности SavePopа проста: мы ничего не собираем и не храним о
...
при использовании функции сохранения, SavePopу может понадобиться проксировать
...
на неизменённом и официальном инстансе SavePopа **все данные туннеля шифруются
...
если исходный код SavePopа не изменён.

медиаданные из туннелей нигде не хранятся/кэшируются. всё обрабатывается в
реальном времени, даже при ремуксинге и транскодировании. туннели SavePopа
...
число активных пользователей SavePopа, полностью анонимно. никакая
...
plausible](https://{env.PLAUSIBLE_HOST}/), который использует SavePop.
